...
//
// `samples/` holds customer drawings and is never committed — nor are their
// names, which is why the sets are addressed by the role they play. Every test
// here returns early when they are absent, and the fixture tests below cover
// the same ground on documents the test wrote itself.
//
...
/// One side of a named pair from `samples/sets.json`, or `None` when the
/// drawings are not on this machine. They are referred to by role rather than
/// by name — see `sc_fixture::samples` for why.
fn sample(set: &str, side: &str) -> Option<String> {
    Samples::load()?.path(set, side)
}
...
fn opens_a_real_set_and_counts_its_sheets() {
    ...
    for set in ["same_producer", "large"] {
        let (Some(p), Some(n)) = (m.path(set, "a"), m.number(set, "sheets")) else {
            continue;
        };
        assert_eq!(Document::open(&p).expect("opens").page_count(), n, "{set}");
    }
}

#[test]
fn rotation_is_already_applied_to_the_page_size() {
    // One side of this pair was exported landscape where the other was
    // portrait, and both draw
    ...
    let (Some(a), Some(d)) = (sample("rotated", "a"), sample("rotated", "b")) else {
        return;
    };
...
fn renders_a_tile_that_matches_the_same_region_of_the_whole_sheet() {
    let Some(p) = sample("same_producer", "a") else {
        return;
    };
...
fn extracts_text_across_producers() {
    // One side of this pair uses subset Type1C/WinAnsi fonts and the other CID
    // TrueType/Identity-H. Sheet matching leans on the text, so it has to come
    // out of both.
    let Some(m) = Samples::load() else { return };
    let Some(label) = m.top("a_net_label") else {
        return;
    };
    for side in ["a", "b"] {
        let Some(p) = m.path("rotated", side) else {
            return;
        };
        let text = Document::open(&p)
            .expect("opens")
            .page_text(2)
            .expect("has a sheet 2");
        assert!(
            text.contains(&label),
            "side {side}: expected a net label in the text"
        );
    }
}
